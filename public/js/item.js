...
function manageRow(data) {
	...
	$.each( data, function( key, value ) {
	  	...
	  	rows = rows + '<td data-id="'+value.id+'">';
                ...
                rows = rows + '<button data-toggle="modal" data-target="#delete-item" class="btn btn-danger delete-item">Delete</button>';
                rows = rows + '</td>';
	  	rows = rows + '</tr>';
	});
...
$(".pdf-submit").click(function(e){
    e.preventDefault();
var form_action = $("#pdf-item").find("form").attr("action");
    ...
    $.ajax({
       dataType: 'json',
        ...
        $(".modal").modal('hide');
        toastr.success('Item Printed Successfully.', 'Success Alert', {timeOut: 5000});
    });
